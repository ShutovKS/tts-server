...
def run_startup_self_checks(runtime_or_settings) -> StartupCheckResult:
    ...
    if hasattr(runtime_or_settings, 'settings') and hasattr(runtime_or_settings, 'core'):
    ...
    log_telegram_event(
        LOGGER,
        level=logging.INFO,
        event="telegram.startup.self_check.starting",
    ...
    if not settings.telegram_allowed_user_ids:
        ...
        log_telegram_event(
            LOGGER,
            level=logging.INFO,
            event="telegram.startup.self_check.allowlist",
    ...
    if core is not None:
        try:
            ...
            if tts_service and hasattr(tts_service, 'is_backend_available'):
                ...
                if not is_available:
                    result.errors.append(f"FATAL: Backend '{settings.backend}' is not available")
                else:
                    result.checks_passed.append(f"backend_available:{settings.backend}")
                    log_telegram_event(
                        LOGGER,
                        level=logging.INFO,
                        event="telegram.startup.self_check.backend",
    ...
    if not is_ffmpeg_available():
        result.errors.append("FATAL: ffmpeg is not available in PATH. Required for audio conversion.")
    else:
        result.checks_passed.append("ffmpeg_available")
        log_telegram_event(
            LOGGER,
            level=logging.INFO,
            event="telegram.startup.self_check.ffmpeg",
    ...
    log_telegram_event(
        ...
        event="telegram.startup.self_check.complete",
...
async def verify_telegram_connectivity(client: TelegramBotClient) -> bool:
    ...
    log_telegram_event(
        LOGGER,
        level=logging.INFO,
        event="telegram.startup.connectivity_check",
    ...
    try:
        ...
        log_telegram_event(
            LOGGER,
            level=logging.INFO,
            event="telegram.startup.connected",
        ...
        log_telegram_event(
            LOGGER,
            level=logging.ERROR,
            event="telegram.startup.connection_failed",
...
async def run_telegram_bot(
    ...
    log_telegram_event(
        LOGGER,
        level=logging.INFO,
        event="telegram.startup.phase.self_checks",
    ...
    for warning in self_check_result.warnings:
        log_telegram_event(
            ...
            event="telegram.startup.warning",
            message=warning,
        )
    ...
    log_telegram_event(
        LOGGER,
        level=logging.INFO,
        event="telegram.startup.phase.initialization",
    ...
    try:
        ...
        log_telegram_event(
            LOGGER,
            level=logging.INFO,
            event="telegram.startup.core_starting",
        ...
        log_telegram_event(
            LOGGER,
            level=logging.INFO,
            event="telegram.startup.components_building",
        ...
        from telegram_bot.job_orchestrator import TelegramJobOrchestrator, TelegramJobPoller, DeliveryMetadataStore
        ...
        log_telegram_event(
            LOGGER,
            level=logging.INFO,
            event="telegram.startup.complete",
        ...
        def shutdown_handler():
            log_telegram_event(
                LOGGER,
                level=logging.INFO,
                event="telegram.shutdown.signal_received",
        ...
        log_telegram_event(
            ...
            event="telegram.polling.exit_status",
        ...
        log_telegram_event(
            LOGGER,
            level=logging.INFO,
            event="telegram.polling.stopping",
        ...
        log_telegram_event(
            LOGGER,
            level=logging.ERROR,
            event="telegram.startup.fatal_error",
...
async def _perform_shutdown(client: TelegramBotClient, runtime: TelegramRuntime) -> None:
    ...
    try:
        ...
        log_telegram_event(
            LOGGER,
            level=logging.INFO,
            event="telegram.shutdown.core_stopped",
        ...
        log_telegram_event(
            LOGGER,
            level=logging.WARNING,
            event="telegram.shutdown.core_error",
    ...
    try:
        ...
        log_telegram_event(
            LOGGER,
            level=logging.INFO,
            event="telegram.shutdown.client_closed",
        ...
        log_telegram_event(
            LOGGER,
            level=logging.WARNING,
            event="telegram.shutdown.client_error",
    ...
    try:
        ...
        log_telegram_event(
            LOGGER,
            level=logging.INFO,
            event="telegram.shutdown.metrics",
    ...
    log_telegram_event(
        LOGGER,
        level=logging.INFO,
        event="telegram.shutdown.complete",
